...
async def force_sub(id):
    ...
    ok = False
    try:
        x = await Aziko(GetParticipantRequest(channel=int(FORCESUB), participant=int(id)))
        left = x.stringify()
        if 'left' in left:
            ok = True
        else:
...
@Aziko.on(events.NewMessage(incoming=True, from_users=AUTH_USERS, pattern="^/msg (.*)"))
async def msg(event):
    ...
    await Aziko.send_message(int(user) , ok )
